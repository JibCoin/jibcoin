...
#if defined(NDEBUG)
# error "Jibcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x59823bfdfb6378cec5bf058619702b0ce0ad52a748c575e9fe7de4d328224d47");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Jibcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jibcoin Signed Message:\n";
...
{
    // Jibcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 15000); // Jibcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Jibcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Jibcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jibcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jibcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa2be40c8bb9a3b848da73bd01c49c09699932b275fb3aeef3f42cce82c8637a8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 03/Jun/2018 Democrats Hope an Asian Influx Will Help Turn Orange County Blue";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 15 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04295f51d88cfa7e1273c99b74178c9a84b686431555b7c5102c17775ca0d4883e84016d29e30644eea1886c03f5f9c82f5c5982cfce8523fee4842ecfe4f64405") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528071711;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084524493;

        if (fTestNet)
        {
            block.nTime    = 1528071687;
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa2be40c8bb9a3b848da73bd01c49c09699932b275fb3aeef3f42cce82c8637a8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb4, 0xd3 }; // Jibcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JibcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JibcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JibcoinMiner(CWallet *pwallet)
{
    printf("JibcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jibcoin-miner");
    ...
    try { loop {
        ...
        printf("Running JibcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JibcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JibcoinMiner, pwallet));
}
